...
| **PHP** | PHP | 8.3 / 8.4 | Runtime, JIT compilation |
| **Frontend** | Vanilla JS + Vite | 6.4 | SPA, router hash, fetch client |
| **Node.js** | Node.js | 20.x | Build, tooling |
| **Base de Datos** | PostgreSQL | 16.2 | RDBMS, PostGIS 3.4 para georreferenciación |
| **Cache** | Redis | 7.x | Feed caching, sessions, queues |
| **ORM** | Eloquent | Laravel built-in | Query builder, eager loading, relationships |
| **HTTP Server** | FrankenPHP | latest | Built-in en Laravel Octane (no nginx) |
| **Testing** | k6 | latest | Load testing, JavaScript DSL, JSON output |

### 2.3 Justificación de k6 como Suite de Pruebas
