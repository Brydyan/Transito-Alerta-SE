...
- [ ] **T0.1** Migration `0020_add_closed_status_to_incidents.sql`:
...
- [ ] **T0.2** Migration `0021_add_decision_columns_to_incidents.sql`:
...
- [ ] **T0.3** Migration `0022_add_incident_pending_approval_notification_type.sql`:
...
- [ ] **T0.4** Migration `0023_add_notes_to_status_history.sql`:
...
- [ ] **T1.1** Crear `CreateRoleDto` (`name: string`, validación NotEmpty)
- [ ] **T1.2** Crear `UpdateRoleDto` (Partial de CreateRoleDto)
- [ ] **T1.3** Crear `SyncPermissionsDto` (`permissions: string[]`)
- [ ] **T1.4** Añadir `create(dto)` en `RolesService` — INSERT en tabla `roles`
- [ ] **T1.5** Añadir `findAll()` en `RolesService` — lista roles con sus permisos
- [ ] **T1.6** Añadir `findOne(id)` en `RolesService` — 404 si no existe
- [ ] **T1.7** Añadir `update(id, dto)` en `RolesService`
- [ ] **T1.8** Añadir `delete(id)` en `RolesService` — 409 si tiene usuarios asignados
- [ ] **T1.9** Añadir `syncPermissions(id, permissions[])` en `RolesService` — DELETE + INSERT en transacción
- [ ] **T1.10** Añadir endpoints en `RolesController`:
...
- [ ] **T1.11** Tests unitarios para `RolesService.syncPermissions` (transacción, idempotencia)

---

## Fase 2 — Organizations extras

- [ ] **T2.1** Añadir `tree()` en `OrganizationsService` — carga relaciones parent/children recursivamente
- [ ] **T2.2** Añadir `formData()` en `OrganizationsService` — retorna `{ roles, geoZones }`
- [ ] **T2.3** Añadir `notifiedFor(lat, lng)` en `OrganizationsService` — reutiliza `GeoZonesService.findZoneForPoint()`
- [ ] **T2.4** Añadir endpoints en `OrganizationsController`:
...
- [ ] **T3.1** Verificar si `deleted_at` existe en tabla `users` (revisar migrations 0001-0018)
  - Si no existe: crear migration `0021_add_deleted_at_to_users.sql`
  - Añadir `@DeleteDateColumn()` a `UserEntity`
- [ ] **T3.2** Añadir `adminCreate(dto)` en `UsersService` — delegar a `InvitationsService.invite()` (flujo T3.6)
- [ ] **T3.3** Añadir `findOne(id)` en `UsersService` con relaciones role + org; 404 si no existe
- [ ] **T3.4** Añadir `adminUpdate(id, dto)` en `UsersService` — permite actualizar `name`, `email`, `role_id`, `organization_id`
- [ ] **T3.5** Añadir `softDelete(id)` en `UsersService` — llama `TypeORM.softDelete(id)`
- [ ] **T3.6** Añadir endpoints en `UsersController`:
...
- [ ] **T4.1** Crear `IncidentApprovalService` en `backend/src/modules/notifications/services/incident-approval.service.ts`:
...
- [ ] **T4.2** Crear `RejectNotificationDto` (`reason: string`, `@MinLength(10)`, `@MaxLength(500)`)
- [ ] **T4.3** Añadir endpoints en `NotificationsController`:
...
- [ ] **T4.4** Test unitario: approve idempotencia (409 si `processed_at` ya existe)
- [ ] **T4.5** Test unitario: reject con claimant activo → `in_progress`; sin claimant → `pending`

---

## Fase 5 — CRUD gaps: Incidents

- [ ] **T5.1** Verificar si `deleted_at` existe en tabla `incidents` (revisar migrations 0001-0018)
  - Si no existe: crear migration `0023_add_deleted_at_to_incidents.sql`
  - Añadir `@DeleteDateColumn()` a `IncidentEntity`
- [ ] **T5.2** Crear `UpdateIncidentDto` (`title?`, `description?`, `category_id?`)
- [ ] **T5.3** Añadir `update(id, dto)` en `IncidentsService` (campos editables solamente)
- [ ] **T5.4** Añadir `softDelete(id)` en `IncidentsService`
- [ ] **T5.5** Añadir endpoints en `IncidentsController`:
...
- [ ] **T6.1** Crear `UpdateAssignmentDto` (`operator_id: string`)
- [ ] **T6.2** Añadir `update(id, dto)` en `AssignmentsService`
- [ ] **T6.3** Añadir `PATCH /assignments/:id` en `AssignmentsController` con `@RequirePermission('UPDATE')`
- [ ] **T6.4** Añadir `findOne(id)` en `CommentsService`
- [ ] **T6.5** Añadir `update(id, dto, userId)` en `CommentsService` — con ownership check + XSS sanitize (igual que store)
- [ ] **T6.6** Crear `UpdateCommentDto` (`content: string`, NotEmpty)
- [ ] **T6.7** Añadir endpoints en `CommentsController`:
...
- [ ] Migrations 0020-0027 aplicadas (0020-0023 schema gaps; 0024+ soft-delete si necesarios)
- [ ] `IncidentStatus` type incluye `'closed'`; `closed` NO aparece en `UpdateIncidentStatusDto`
- [ ] `GET /roles` lista con permisos por rol
- [ ] `PUT /roles/:id/permissions` reemplaza permisos en transacción atómica
- [ ] `GET /organizations/tree` devuelve jerarquía anidada
- [ ] `POST /notifications/:id/approve` → incidente pasa a `closed`, decision columns escritos, siblings marcados processed
- [ ] `POST /notifications/:id/reject` → incidente revierte a `in_progress` (con claimant) o `pending` (sin claimant), reason como Comment de auditoría
- [ ] `POST /notifications/:id/approve` con notificación ya procesada → 409
- [ ] `PATCH /incidents/:id` acepta title/description/category_id, rechaza status/zone_id
- [ ] `DELETE /incidents/:id` retorna 204 y soft-delete
