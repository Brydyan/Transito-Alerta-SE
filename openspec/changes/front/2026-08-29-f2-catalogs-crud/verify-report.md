...
| Catalogs e2e run in CI, not just locally | `.github/workflows/ci.yml:456` passes `BASE_URL: ${{ vars.STAGING_BASE_URL }}` | ✅ Confirmed at that exact location — the local `test.skip(!BACKEND_URL, …)` (`BACKEND_URL = process.env['BASE_URL']?.trim()`) is the documented D4 pattern of the e2e-credentials phase, not an F2 defect. |
