...
- [ ] 0.1 `backend/src/config/mail.config.ts`: add `appBaseUrl: string` sourced from
      `process.env.FRONTEND_BASE_URL ?? 'http://localhost:3000'` to `MailConfig` (no new env var —
      see Corrections table).
- [ ] 0.2 `backend/src/config/auth.config.ts`: add `bcryptCost: number` from env `BCRYPT_COST`
      ...
      `# ---- Auth ----`.
- [ ] 0.3 Add `bcrypt` + `@types/bcrypt` to `backend/package.json`.
- [ ] 0.4 Create `backend/src/modules/auth/auth-errors.ts` (mirrors `session-errors.ts` pattern):
      `INVALID_CREDENTIALS`, `INVALID_CREDENTIAL_SHAPE`, `INVALID_TOKEN`, `EMAIL_ALREADY_CLAIMED`.
- [ ] 0.5 Create `backend/src/modules/invitations/invitation-errors.ts`: `INVITATION_NOT_FOUND`,
...
- [ ] 1.1 Create `database/migrations/0017_users_password_identity.sql` — verbatim per design:
...
- [ ] 1.2 Create `database/rollback/0017_users_password_identity.DOWN.sql` — drop `password_hash`;
      restore both `SET NOT NULL`, failing loudly (not inventing a UUID) if any `device_uuid IS
      NULL` row exists on either table.
- [ ] 1.3 Create `database/migrations/0018_invitations.sql` — `invitations` + `password_reset_tokens`
...
- [ ] 1.4 Create `database/rollback/0018_invitations.DOWN.sql` — drop `password_reset_tokens` before
      `invitations` (FK order), remove the 3 permission rows and the role-matrix append.
- [ ] 1.5 Apply 0017 + 0018 to local dev Postgres twice each (idempotence); confirm existing
      `users`/`user_sessions` rows survive unchanged, `password_hash IS NULL` everywhere.
- [ ] 1.6 Register 0017/0018 in `database/MIGRATION_LOG.md` (status Pending until applied to
      Supabase).

## Phase 2: Entity & Repository Infrastructure

- [ ] 2.1 `backend/src/entities/user.entity.ts`: `deviceUuid!: string | null` (drop the implicit
      non-null), add `@Column({ name: 'password_hash', type: 'char', length: 60, nullable: true })
      passwordHash!: string | null;`.
- [ ] 2.2 `backend/src/entities/user-session.entity.ts`: `deviceUuid` becomes `string | null` to
      match 0017's `ALTER TABLE user_sessions`.
- [ ] 2.3 Create `backend/src/entities/invitation.entity.ts` mapping all 0018 `invitations` columns.
- [ ] 2.4 Create `backend/src/entities/password-reset-token.entity.ts` mapping all 0018
      `password_reset_tokens` columns.
- [ ] 2.5 Create `backend/src/modules/invitations/invitations.repository.ts` (raw SQL via
...
- [ ] 2.6 Create `backend/src/modules/auth/password-reset.repository.ts`: `insert`, `casConsume`
      (same CAS shape on `used_at`), `findDiagnosisByHash`.
- [ ] 2.7 `backend/src/modules/sessions/sessions.repository.ts`: add `updatedRows<T>()` helper
...
- [ ] 3.1 Create `backend/src/modules/auth/token-codec.ts`: `generateToken()` (32 random bytes →
...
- [ ] 3.2 Create `backend/src/modules/auth/password-hasher.ts` (`PasswordHasher`, injectable): cost
...
- [ ] 3.3 Create `backend/src/modules/auth/credential-dispatch.ts`: `resolveCredential(dto):
...
- [ ] 3.4 Create `backend/src/common/authz/assert-can-invite.ts`: `assertCanInvite(actor,
...
- [ ] 3.5 Create `backend/src/modules/invitations/invitations.service.ts`: `createInvitation` (409
...
- [ ] 3.6 Create `backend/src/modules/auth/password-reset.service.ts`: `requestReset(email)` (silent
...
- [ ] 4.1 `backend/src/modules/auth/auth.service.ts`: extract `private issueSession(user, deviceUuid:
...
- [ ] 4.2 Add `AuthService.loginWithPassword({ email, password, deviceUuid }, meta)`: `SELECT ...
...
- [ ] 4.3 Add `AuthService.revokeAllForUser(userId)`: `sessionsRepository.revokeAllForUser(userId)`
...
- [ ] 4.4 D8 null guards: `getPermissions(deviceUuid: string | null)` returns `[]` immediately when
...
- [ ] 4.5 Add `AuthService.changePassword(userId, currentPassword, newPassword)`: load user, `401
...
- [ ] 5.1 Rewrite `backend/src/modules/auth/dto/login.dto.ts`: one class, 3 optional fields
...
- [ ] 5.2 Create `backend/src/modules/auth/dto/accept-invitation.dto.ts`: `{ token: string, password:
      string }` with `@MinLength(authConfig.passwordMinLength)` (12).
- [ ] 5.3 Create `backend/src/modules/auth/dto/password-reset-request.dto.ts`: `{ email: string
      (@IsEmail()) }`.
- [ ] 5.4 Create `backend/src/modules/auth/dto/password-reset-confirm.dto.ts`: `{ token: string,
      password: string (@MinLength(12)) }`.
- [ ] 5.5 Create `backend/src/modules/auth/dto/change-password.dto.ts`: `{ current_password: string,
      new_password: string (@MinLength(12)) }`.
- [ ] 5.6 `auth.controller.ts` `login()`: call `resolveCredential(dto)`, dispatch to
...
- [ ] 5.7 Add `POST /api/auth/accept-invitation` → `InvitationsService.redeem(token, password)`,
      `201`, returns `AuthTokens`.
- [ ] 5.8 Add `POST /api/auth/password-reset` → `PasswordResetService.requestReset(email)`,
...
- [ ] 5.9 Add `POST /api/auth/password-reset/confirm` → `PasswordResetService.confirmReset(token,
      password)`, `200`.
- [ ] 5.10 Add `PUT /api/auth/password` (`@UseGuards(JwtAuthGuard)`, no `PermissionGuard` — SELF-only
      per spec, mirrors T3.9's self-session-revoke bypass) → `AuthService.changePassword(userId,
      current_password, new_password)`.
- [ ] 5.11 `GET /api/auth/me` return type widens to `{ user_id, device_uuid: string | null,
...
- [ ] 5.12 `auth.module.ts`: `imports: [..., InvitationsModule, MailModule]` (`PasswordResetService`
      lives in `auth/` per design — its only consumer is `AuthController`, but it needs
      `MailService`).

## Phase 6: Invitations Controller & Routes

- [ ] 6.1 Create `backend/src/modules/invitations/dto/create-invitation.dto.ts`: `{ email
      (@IsEmail()), role_id (@IsUUID()), organization_id (@IsUUID(), optional) }`.
- [ ] 6.2 Create `backend/src/modules/invitations/invitations.controller.ts` (`@Controller()`, no
...
- [ ] 6.3 Create `backend/src/modules/invitations/invitations.module.ts`: providers
...
- [ ] 6.4 `backend/src/app.module.ts`: register `InvitationsModule`.
- [ ] 6.5 `backend/src/modules/users/users.service.ts`: audit every `deviceUuid` read for the D8
      null-tolerance contract (this task implements the fix; 9.x re-verifies via grep).

## Phase 7: Mail & Templates

- [ ] 7.1 `backend/src/modules/mail/templates/mail-templates.ts`: extend `TemplateName` with
...
- [ ] 7.2 `InvitationsService.createInvitation` / `PasswordResetService.requestReset`: build the
      subject line inline (precedent: `IncidentMailListener.subjectFor`) and call
      `MailService.enqueue({ to, subject, template, data })`.
- [ ] 7.3 Unit test `renderMailTemplate('invitation', ...)` / `('password-reset', ...)` render
      escaped HTML with no unescaped token/email injection.

## Phase 8: Testing

- [ ] 8.0 Run full existing suite (`npm test`, `npm run test:e2e` from `backend/`); record baseline
      614/614 unit, 122/122 e2e before writing a single new test.
- [ ] 8.1 Unit: `resolveCredential` — device / password / neither / both (table-driven, pure).
- [ ] 8.2 Unit: `PasswordHasher` — round-trip, `DUMMY_HASH` verifies false against any real password,
      cost is read from config (assert cost 4 is honored under a test override, never a hardcoded
      12).
- [ ] 8.3 Unit: `AuthService.loginWithPassword` — unknown email, null `password_hash`, inactive user,
      success; assert `bcrypt.compare` is invoked on every branch (the timing-equalization assertion,
      not a comment).
- [ ] 8.4 Unit: `AuthService.login` device path — run the EXISTING spec unmodified (see 4.1); this is
      the regression gate for the `issueSession` extraction, not a new test.
- [ ] 8.5 Unit: `InvitationsService` — 404-vs-410 diagnosis, `assertCanInvite` rank/scope matrix,
      claimed-email 409 pre-check. Mocked repository.
- [ ] 8.6 Unit: `PasswordResetService` — silent-miss request, confirm CAS, token reuse → 410, bulk
      revoke delegated to `AuthService.revokeAllForUser`. Mocked repository.
- [ ] 8.7 Unit: `token-codec` (`generateToken`/`decodeTokenOrThrow`) — pure.
- [ ] 8.8 Integration (Testcontainers Postgres): `SessionsRepository.revokeAllForUser` — N active + 1
      already-revoked + 1 expired row → returns exactly the N active ones, unwrapped via
      `updatedRows`, never `firstUpdatedRow`.
- [ ] 8.9 Integration (Testcontainers Postgres, real concurrency): `InvitationsService.redeem` CAS —
      `Promise.all` of two `redeem()` calls on one token → exactly one `201` + one `users` row, one
      `410 INVITATION_ALREADY_USED`.
- [ ] 8.10 Integration (Testcontainers Redis): `MailService.enqueue` puts `invitation` /
      `password-reset` on `mail:outbox` with a rendered, escaped body.
- [ ] 8.11 E2E (`backend/test/e2e/invitations.e2e-spec.ts`): full flow — invite → preview → accept
...
- [ ] 8.12 E2E: expiry via SQL `UPDATE` in test setup only (`expires_at`/`accepted_at`/`used_at`
      backdated directly — never `sleep`/clock injection, house rule); expired invite → `410` at
      preview and at `accept-invitation`.
- [ ] 8.13 E2E: concurrent redemption (`Promise.all`, two requests, same token) → one `201`, one `410
      INVITATION_ALREADY_USED`, never `409`.
- [ ] 8.14 E2E: invalid tokens — unknown → `404` at preview, malformed base64url → `400
      INVALID_TOKEN`, reused → `410`.
- [ ] 8.15 E2E: duplicate invitation to an already-claimed email → `409` at creation, no row created.
- [ ] 8.16 E2E: unauthorized/malformed invite creation → `403` (no `invitations:CREATE`) and `422`
      (malformed email) — no `invitations` row in either case.
- [ ] 8.17 E2E: `PUT /auth/password` — wrong `current_password` → `401`; success revokes every
      session including the caller's own (D5 — next request on the SAME token also `401`).
- [ ] 8.18 Add `test-environment.ts` helpers: `provisionPasswordUser(email, password)`,
      `backdateInvitation(id, secondsAgo)` / `backdateResetToken(id, secondsAgo)` via SQL `UPDATE`.
- [ ] 8.19 Regression: run the FULL suite once more; confirm all 122 pre-existing e2e tests pass
      unmodified (device path untouched) — any pre-existing test needing an edit is a design bug,
      not a task to "fix".

## Phase 9: Verification Obligations

- [ ] 9.1 `grep -rn "password_hash" backend/src` — confirm no plaintext password is ever logged,
      returned in a DTO, or compared with `===`.
- [ ] 9.2 `grep -rn "user_sessions" backend/src` — confirm the only writers remain
      `sessions/sessions.repository.ts` and `entities/user-session.entity.ts` (T3.9 invariant,
      unchanged by this capability).
- [ ] 9.3 Confirm every `password_hash` write (`redeem`, `confirmReset`, `changePassword`) is
      immediately followed by a `revokeAllForUser` call in the same service method.
- [ ] 9.4 `grep -rn "setTimeout\|sleep(" backend/test/e2e/invitations.e2e-spec.ts` — zero matches;
      all expiry cases use SQL `UPDATE`.
- [ ] 9.5 `grep -rn "bcrypt.compare\|bcrypt.hash" backend/src` — confirm every password comparison
      goes through `PasswordHasher`/`bcrypt.compare`, never a raw `===` on `password_hash`.
- [ ] 9.6 `grep -rn "\.deviceUuid" backend/src/modules/{auth,users,realtime}` — confirm every read
