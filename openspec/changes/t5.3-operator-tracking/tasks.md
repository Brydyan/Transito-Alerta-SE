...
- [x] 0.1 Create `backend/src/modules/operators/` directory.
- [x] 0.2 Create `backend/src/modules/operators/operators.module.ts` with `@Module({})` stub.
...
- [x] 0.3 Add `OperatorsModule` to `AppModule` imports.

## Phase 1: Constants + DTOs

- [x] 1.1 Create `backend/src/modules/operators/operator-role.constants.ts` with:
...
- [x] 1.2 Create `backend/src/modules/operators/dto/update-location.dto.ts` with `@IsNumber()
      @Min(-90) @Max(90) lat` and `@IsNumber() @Min(-180) @Max(180) lng`.
- [x] 1.3 Create `backend/src/modules/operators/dto/dashboard-query.dto.ts` with `inicio?`,
      `fin?` (IsDateString), `location_id?` (IsUUID), `page?` (IsInt, min 1), `per_page?`
      (IsInt, 1–50).
- [x] 1.4 Create `backend/src/modules/operators/dto/operator-location.dto.ts` with `userId`,
      `organizationId`, `lat`, `lng`, `updatedAt` (ISO string).
- [x] 1.5 Create `backend/src/modules/operators/dto/operator-dashboard-response.dto.ts` with
      `stats: {total_assigned, in_progress, resolved_today}`, `incidents: DashboardIncidentDto[]`,
      `pagination: PaginationMeta`.

## Phase 2: Location Service

- [x] 2.1 Create `backend/src/modules/operators/operator-location.service.ts` injecting the
...
- [x] 2.2 Implement `record(userId: string, orgId: string, lat: number, lng: number): Promise<void>`:
...
- [x] 2.3 Implement `activeFor(orgId: string | null, isSystemAdmin: boolean): Promise<OperatorLocationDto[]>`:
...
- [x] 3.1 `backend/src/modules/operators/operator-location.service.spec.ts`:
...
- [x] 4.1 Create `backend/src/modules/operators/operator-dashboard.service.ts` injecting `DataSource`.
- [x] 4.2 Implement `forOperator(userId: string, filters: DashboardQueryDto): Promise<OperatorDashboardResponseDto>`:
...
- [x] 5.1 `backend/src/modules/operators/operator-dashboard.service.spec.ts`:
...
- [x] 6.1 Create `backend/src/modules/operators/operators.controller.ts`:
...
- [x] 6.2 Implement role guard logic in controller: check `user.roleName` against `OPERATOR_PING_ROLES`
      or `OPERATOR_QUERY_ROLES`; system admin passes all checks.
- [x] 6.3 Register controller and both services in `OperatorsModule`.
- [x] 6.4 Add `@Controller('operator')` prefix to `OperatorsController`.

## Phase 7: E2E Tests

- [x] 7.1 `backend/test/e2e/operator-tracking.e2e-spec.ts`:
...
- [x] 8.1 `npm run lint` — zero new violations.
- [x] 8.2 `npm run typecheck` — no errors.
- [x] 8.3 `npm run build` — clean.
- [x] 8.4 `npm test && npm run test:e2e` — full suite green.
