...
> **Excepción**: typecheck va crudo — `npx tsc -b tsconfig.json --noEmit`. `rtk tsc` da
> falso negativo, comprobado en la 3ª auditoría de F0. `tsc -p` sin `-b` también
> devuelve 0 falso porque `tsconfig.json` usa `references` con `files: []`.

---

## T1 — Utilidades de cálculo

- [x] **T1.1** — `parseThemeTokens(cssText: string): Record<string, string>`: extrae los
...
- [x] **T1.2** — `relativeLuminance(hex): number` según WCAG 2.x: sRGB normalizado →
      linealización (`c ≤ 0.03928 ? c/12.92 : ((c+0.055)/1.055)^2.4`) → ponderación
      `0.2126 R + 0.7152 G + 0.0722 B`.
- [x] **T1.3** — `blend(fg, alpha, bg): string`: composición canal a canal
      `α·fg + (1−α)·bg`. El fondo por defecto es `--color-bg-secondary` leído de los
      tokens, **no** el literal `#FFFFFF` — D3.
- [x] **T1.4** — `contrastRatio(a, b): number`: `(L_claro + 0.05) / (L_oscuro + 0.05)`.

## T2 — Autovalidación de la fórmula (va ANTES de medir los pares reales)

- [x] **T2.1** — Test que verifica `contrastRatio('#000000', '#FFFFFF') === 21` con
      tolerancia 0.01.
- [x] **T2.2** — Test que verifica `contrastRatio('#FFFFFF', '#FFFFFF') === 1`.
- [x] **T2.3** — Test contra un par publicado por WCAG con razón conocida.
- [x] **T2.4** — Test de `blend`: `blend('#000000', 0, '#FFFFFF')` da blanco;
...
- [x] **T3.1** — Declarar `BADGE_PAIRS: Record<UiBadgeVariant, ContrastPair>` con las 8
      variantes. Tipar con `Record<UiBadgeVariant, …>` para que **falte una sea error de
      compilación**, no sólo de runtime.
- [x] **T3.2** — Declarar `KPI_PAIRS: Record<UiKpiTone, ContrastPair>` con los 7 tonos,
      mismo tipado.
- [x] **T3.3** — Test de completitud en runtime: recorrer las claves y afirmar que
...
- [x] **T4.1** — `it.each` sobre los 8 pares de `ui-badge`: contraste **≥ 4.5**. El
      mensaje de fallo incluye variante, tokens, alfa y razón obtenida.
- [x] **T4.2** — `it.each` sobre los 7 pares de `ui-kpi-card`: mismo umbral, mismo
      formato de mensaje.
- [x] **T4.3** — Verificá que las variantes tintadas se componen: `pendiente` (`/20`),
      `resuelto` (`/15`), `cerrada` (`/12`), `low` (`/15`), `medium` (`/40`),
      `high` (`/15`). `en_proceso` usa `bg-brand-primary-soft`, que es un token sólido —
      sin alfa.
- [x] **T4.4** — **Probá que el test detecta.** Bajá temporalmente un token de
...
- [x] **T5.1** — Retirar las razones puntuales de contraste de
...
- [x] **T5.2** — Grep de verificación: no deben quedar coincidencias de razones con
...
- [x] **T6.1** — `rtk jest` desde `frontend/`: **todo verde**. Baseline al cerrar F0: 192
      tests. Este change los sube; ninguno debe romperse.
- [x] **T6.2** — `npx tsc -b tsconfig.json --noEmit` **crudo**: exit 0.
  Importante: `tsconfig.json` usa `references` (`files: []`), por lo que
  `tsc -p` sin `-b` no procesa archivos y devuelve 0 aunque haya errores
  reales. El `-b` es obligatorio. `rtk tsc` queda descartado (falso
  negativo conocido de F0, R3.1).
- [x] **T6.3** — `rtk npm run build`: verde.
- [x] **T6.4** — `rtk git diff --stat`: cero cambios bajo `backend/`, `database/` y
      `openspec/specs/`.
...
- [x] Los 15 pares verificados con cálculo real, ≥ 4.5:1
- [x] Ningún hex embebido en el test: todo sale de `_variables.css`
- [x] Agregar una variante o un tono sin declarar su par **rompe la compilación o el test**
- [x] La fórmula validada contra 3 razones conocidas, antes de las aserciones reales
- [x] El test se vio fallar de verdad (T4.4), documentado en `apply-progress.md`
- [x] Cero razones puntuales de contraste en los comentarios de los cuatro archivos
- [x] `rtk jest` verde, `npx tsc` exit 0, `rtk npm run build` verde
