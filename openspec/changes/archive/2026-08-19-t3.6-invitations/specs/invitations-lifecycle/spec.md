...
`POST /api/auth/password-reset {email}` MUST always respond `200` regardless of whether the email matches an account, and MUST email a single-use 24h token only when it does.
...
A duplicate invitation to an already-claimed email at creation time MUST `409`. Malformed payloads (invalid email, missing password, password under the length floor) MUST `422`.
...
- AND the malformed-email request receives `422`
