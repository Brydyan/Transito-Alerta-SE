...
- Scenario: Roles agrupados — GIVEN la respuesta de la matriz THEN los roles se
    presentan separados en tres bloques por `scope`: «empresa principal»
    (`platform`), «organizaciones» (`organization`) y «público» (`public`)
...
- Scenario: Ámbito obligatorio — GIVEN la creación de un rol THEN su `scope` es
  `platform`, `organization` o `public`; no existe otro estado ni un rol sin
  clasificar
- Scenario: Migración de roles existentes — GIVEN la migración aplicada THEN
  `master` y `operador_sistema` quedan como `platform`, `admin_org` y
  `operador_org` como `organization` (DEFAULT), y `reporter` como `public`
