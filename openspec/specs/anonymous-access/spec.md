# Specification: Anonymous Access

**Capability**: `anonymous-access` — qué puede hacer la identidad anónima
(`device_uuid = 'anonymous'`). Desde 2026-09-05: nada.

Origen: change `2026-09-02-anon-close-anonymous-reporting` (sc-326). El dominio se declaró
como `(MODIFIED)` en el delta, pero nunca existió como spec propio: el techo anónimo vivía
sólo en `auth.config.ts` y en los escenarios de `e2e-flows`. Este archivo lo hace explícito.

La fila máscara de la base **no se borra**: AUD la recicla como identidad de publicación
para los reportes anónimos del ciudadano autenticado.
