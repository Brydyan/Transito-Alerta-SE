...
| 0032 | updated_at_columns | `updated_at TIMESTAMPTZ NOT NULL DEFAULT now()` on 15 tables + function `set_updated_at()` + BEFORE UPDATE trigger per table (T7.3) | ⏳ Pending | — | — | — |
| 0033 | comments_threading | `comments.parent_id uuid REFERENCES comments(id) ON DELETE CASCADE`, CHECK `parent_id IS DISTINCT FROM id`, index on `parent_id`, depth-2 limit enforced in app layer (T7.4) | ⏳ Pending | — | — | — |
| 0034 | organizations_hierarchy_categories | `organizations.parent_id uuid REFERENCES organizations(id) ON DELETE SET NULL` + CHECK, `organizations.incident_category_id uuid REFERENCES incident_categories(id)`, removes `uq_organizations_zone` UNIQUE constraint (T7.5) | ⏳ Pending | — | — | — |
| 0035 | domain_columns | `geo_zones.code varchar(32) NULL` with partial UNIQUE index, `users.phone varchar(30) NULL` (T7.6) | ⏳ Pending | — | — | — |
| 0036 | referential_integrity | Function `check_is_leaf_category()` + trigger on incidents to enforce leaf-category-only; 4 FK normalized with explicit ON DELETE clauses (T7.7) | ⏳ Pending | — | — | — |
| 0037 | index_parity | 4 missing indexes: `comments.user_id`, `comments.parent_id`, `assignments.incident_id`, `status_history.changed_by_user_id` (T7.8) | ⏳ Pending | — | — | — |
| 0038 | reference_data | 22-category tree seed (5 roots + 17 leaves) with idempotent `ON CONFLICT DO NOTHING` + 2 partial UNIQUE indexes for idempotence (T7.9.A) | ⏳ Pending | — | — | — |
...
| 0040 | rename_roles | Rename for clarity: `admin_sistema` → `master`, `admin_organizacion` → `admin_org`, `operador_organizacion` → `operador_org` (T7.10) | ⚠️ Ejecutada en Supabase, **sin registrar** en `schema_migrations` — el próximo `db:migrate` la re-aplica como no-op idempotente y la registra | Andy Alejandro | 2026-08-25 | supabase |
