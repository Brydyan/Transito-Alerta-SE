...
| 0039 | organizations_permissions | Permission rows `(notifications, READ)` + `(notifications, UPDATE)` added to catalog + granted to 4 staff roles via `roles.permissions` JSONB (T7.9.B); partial org seeding pending operator input (T7.9.C) | ⏳ Pending | — | — | — |
